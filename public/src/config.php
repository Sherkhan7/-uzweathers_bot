...
function connect() {
    ...
    if (count($func_args) < 4 || count($func_args) > 5) {
        ...
        file_put_contents(__DIR__ . '/PDOException' . '/connection_args.json', "\n\nLess or more arguments");
        return null;

    } else {

        file_put_contents(__DIR__ .'/PDOException' . '/connection_args.json', "\n\n". json_encode($func_args, JSON_PRETTY_PRINT));
    }
    $host = $func_args[0];
    $dbname = $func_args[1];
    ...
    try {
        ...
        file_put_contents(__DIR__ .'/PDOException' . '/PDOException.txt' , "\n\n" . $e->getMessage(), FILE_APPEND);
    }
